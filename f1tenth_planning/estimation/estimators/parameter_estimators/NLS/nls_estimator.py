...
class NLSParameterEstimator(ParameterEstimator):
    def __init__(
        ...
        self._opti = self.__initialize_nls_problem(
            initial_params, model.nx, model.nu, estimation_history_length
        )
    ...
    def estimate(self, state: dict, control: np.ndarray) -> dynamics_config:
        ...
        x_curr = np.array(
            [x, y, state["delta"], v, yaw, state["ang_vel_z"], state["beta"]]
        )  # Currently only support dynamic-bycicle model
